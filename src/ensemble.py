...
class EnsembleModel:
    def __init__(self, data_path, processed_data_path, plot_dir='./plot/ensemble'):
    ...
    def load_data(self, sample_fracs, random_state=42):
        df = pd.read_csv(self.data_path, header=0, dtype=str)
    ...
    def tune_hyperparameters(self, X_train, y_train, model, params):
        grid_search = GridSearchCV(model, params, cv=5, n_jobs=-1, verbose=2)
        grid_search.fit(X_train, y_train)
        return grid_search.best_estimator_

    def plot_learning_curves(self, model, X_train, y_train):
    ...
    def plot_roc_curve(self, y_test, y_pred_prob, n_classes):
        plt.figure()
    ...
    def plot_precision_recall_curve(self, y_test, y_pred_prob, n_classes):
        plt.figure()
        for i in range(n_classes):
    ...
    def plot_permutation_importance(self, model, X_train, y_train):
    ...
    def train_and_evaluate(self):
        ...
        X, y = self.load_data(self.data_path, sample_fracs)
        ...
        best_rf = self.tune_hyperparameters(X_train_balanced, y_train_balanced, RandomForestClassifier(random_state=42), rf_params)
        ...
        best_xgb = self.tune_hyperparameters(X_train_balanced, y_train_balanced, XGBClassifier(random_state=42), xgb_params)
        ...
        # Learning curves
        self.plot_learning_curves(ensemble, X_train_balanced, y_train_balanced)
        ...
        self.plot_roc_curve(y_test, y_pred_prob, n_classes)

        # Plot Precision-Recall Curve
        self.plot_precision_recall_curve(y_test, y_pred_prob, n_classes)

        # Plot Feature Importance for RandomForest
        self.plot_feature_importance(best_rf, X_train_balanced)

        # Plot Permutation Importance for Ensemble
        self.plot_permutation_importance(ensemble, X_train_scaled, y_train)
